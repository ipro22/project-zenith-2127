...
export default function AccountPage() {
  ...
  const [emailMode, setEmailMode] = useState<"login" | "register" | "reset">(
    "login",
  );
  ...
  const loadProfile = useCallback(async (t: string) => {
    try {
      const data = await apiGet<{ client: ClientData }>(
        API.auth,
        { action: "get_profile" },
        t,
      );
  ...
  const handleSendCode = async () => {
    if (phone.replace(/\D/g, "").length < 10) {
      setAuthError("Введите корректный номер");
      return;
    }
    setAuthError("");
    setAuthLoading(true);
    try {
      const data = await apiPost<{ success: boolean; dev_code?: string }>(
        API.auth,
  ...
  const handleVerify = async () => {
    ...
    try {
      const data = await apiPost<{
        ...
        token: string;
        client: ClientData;
      }>(API.auth, { action: "verify_code", phone, code });
  ...
  const handleEmailSend = async () => {
    ...
    try {
      const data = await apiPost<{ success: boolean; dev_code?: string }>(
        API.auth,
        { action: "email_register", email: emailValue },
      );
      setEmailCodeSent(true);
      if (data.dev_code) setEmailDevCode(data.dev_code);
    } catch {
      setAuthError("Ошибка отправки кода");
    }
    setAuthLoading(false);
  };
  ...
  const handleResetConfirm = async () => {
    ...
    try {
      ...
      setAuthError("Неверный код или ошибка сервера");
    }
    setAuthLoading(false);
  };
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setPasswordError("Ошибка. Попробуйте позже.");
    }
    setPasswordSaving(false);
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      await apiPost(
        API.auth,
        { action: "update_profile", name: profileName, email: profileEmail },
        token,
      );
      setSaved(true);
      setClient((c) =>
        c ? { ...c, name: profileName, email: profileEmail } : c,
      );
  ...
  const handleTrack = async () => {
    ...
    setTrackResult(null);
    try {
      const data = await apiPost<{ order?: Order; error?: string }>(
        API.orders,
        {
          action: "check_status",
          order_number: trackNumber.trim().toUpperCase(),
        },
      );
      if (data.order) setTrackResult(data.order);
      else setTrackError("Заказ не найден. Проверьте номер.");
    } catch {
      setTrackError("Ошибка запроса. Попробуйте позже.");
    }
    setTrackLoading(false);
  };

  const level = client?.loyalty_level || "standard";
  const nextLevelVisits =
    level === "standard" ? 3 : level === "regular" ? 5 : null;
  ...
  if (!client) {
    return (
      <div className="min-h-screen bg-gray-50">
        <SEOHead
          title="Личный кабинет — iPro Барнаул"
          description="Войдите в личный кабинет iPro для отслеживания заказов и управления бонусами."
        />
        <Navbar />
          ...
          <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-3">
                        <div className="relative">
                          ...
                          <input
                            ...
                            onKeyDown={(e) =>
                              e.key === "Enter" && handleLoginPassword()
                            }
                            className="w-full pl-10 pr-4 py-3 rounded-2xl border border-gray-200 bg-gray-50 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all"
                          />
                        </div>
                          ...
                          <>
                            ...
                            {resetDevCode && (
                              <div className="flex items-center gap-2 p-3 bg-amber-50 rounded-xl border border-amber-100">
                                <AlertCircle className="w-4 h-4 text-amber-500 shrink-0" />
                                <p className="text-xs text-amber-700">
                                  Тестовый код: <strong>{resetDevCode}</strong>
                                </p>
                              </div>
                            )}
                            <input
            ...
            <div className="flex flex-col gap-5">
              ...
              >
                ...
                {trackError && (
                  <p className="text-red-500 text-xs mt-2">{trackError}</p>
                )}
                {trackResult && (
                  <div className="mt-4 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                    <p className="font-semibold text-gray-900 text-sm">
                      {trackResult.device_brand} {trackResult.device_model}
                    </p>
                    ...
                    <div className="mt-3">
                          ...
                          return (
                            ...
                            >
                              ...
                              {i < statusSteps.length - 1 && (
                                <div
                                  className={`flex-1 h-0.5 ${i < curr ? "bg-blue-600" : "bg-gray-200"}`}
                                />
                              )}
                            </div>
                          );
                      ...
                      <span
                        className={`inline-block text-xs px-2.5 py-1 rounded-full font-medium ${statusColors[trackResult.status]}`}
                      >
              ...
              >
                ...
                <div className="space-y-2.5 text-sm text-blue-100">
                  {[
                    {
                      icon: Shield,
                      text: "Стандарт — 5% бонусами от каждого ремонта",
                    },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <SEOHead
        title="Личный кабинет — iPro Барнаул"
        description="Личный кабинет iPro: ваши заказы, бонусы и профиль."
      />
      <Navbar />
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`text-xs px-3 py-1.5 rounded-full font-semibold ${loyaltyColors[level]}`}
                >
                  {loyaltyLabels[level]}
                </span>
                <button
            ...
            <div className="mt-5 grid grid-cols-3 gap-3">
              {[
                {
                  label: "Бонусов",
                  value: (client.bonus_balance || 0).toLocaleString("ru"),
                  color: "text-blue-600",
                  bg: "bg-blue-50",
                },
                {
                  label: "Ремонтов",
                  value: client.visits_count || 0,
                  color: "text-green-600",
                  bg: "bg-green-50",
                },
                {
                  label: "Скидка",
                  value: loyaltyDiscounts[level],
              ...
              <div className="mt-4">
                <div className="flex justify-between text-xs text-gray-500 mb-1.5">
                  <span>
                    Прогресс до уровня «
                    {level === "standard"
                      ? loyaltyLabels["regular"]
                      : loyaltyLabels["vip"]}
                    »
                  </span>
                  <span>
                    {client.visits_count || 0}/{nextLevelVisits}
                  </span>
                </div>
                <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(((client.visits_count || 0) / nextLevelVisits) * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
                  ...
                  {trackResult && (
                    <div className="mt-4 p-4 bg-gray-50 rounded-2xl">
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`text-xs px-2.5 py-1 rounded-full font-medium ${statusColors[trackResult.status]}`}
                        >
                          {statusLabels[trackResult.status]}
                        </span>
                      </div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 my-3">
                        {statusSteps.map((step, i) => {
                          ...
                          return (
                            ...
                            >
                              <div
                                className={`w-2.5 h-2.5 rounded-full ${active ? "bg-blue-600" : "bg-gray-200"}`}
                              />
                              {i < statusSteps.length - 1 && (
                                <div
                                  className={`flex-1 h-0.5 ${i < curr ? "bg-blue-600" : "bg-gray-200"}`}
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                        <span className="text-sm font-bold text-gray-900">
                          {(order.service_price || 0).toLocaleString("ru")} ₽
                        </span>
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    ...
                    {
                      label: "Уровень",
                      value: loyaltyLabels[level],
                      sub: `Скидка ${loyaltyDiscounts[level]}`,
                      icon: Star,
                      color: "bg-amber-500",
                    },
                    {
                    ...
                    >
                      <div
                        className={`w-9 h-9 rounded-xl ${c.color} flex items-center justify-center mb-3`}
                      >
                ...
                {nextLevelVisits && (
                  <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5">
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                      Ещё {nextLevelVisits - (client.visits_count || 0)}{" "}
                      обращений до уровня «
                      {level === "standard"
                        ? loyaltyLabels["regular"]
                        : loyaltyLabels["vip"]}
                    ...
                    <div className="divide-y divide-gray-50">
                      {bonusTx.map((tx, i) => (
                        ...
                        >
                          ...
                          <span
                            className={`text-sm font-bold ${tx.type === "earn" ? "text-green-600" : "text-red-500"}`}
                          >
                            {tx.type === "earn" ? "+" : "-"}
                            {tx.amount}
                          </span>
                ...
                <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
                  ...
                  {!client.has_password && (
                    <p className="text-xs text-gray-400 mb-4">
                      Добавьте пароль, чтобы входить по email и паролю (удобнее
                      без SMS-кодов)
                    </p>
                  )}
                  <div className="space-y-3">
                ...
                <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      ...
                      {
                        label: "Потрачено",
                        value: `${Math.round((client.total_spent || 0) / 1000)}к ₽`,
                      },
